<template>
  <div ref="reviewModal" class="modal fade review-modal" id="exampleModalCenteredScrollable" tabindex="-1"
...
export default {
  ...
  computed: {
    currentImdbId() {
      return this.selectedImdbId || this.movieInfo?.imdbID || ''
    }
  },
  mounted() {
    this.$refs.reviewModal?.addEventListener('show.bs.modal', this.loadReview)
  },
  beforeUnmount() {
    this.$refs.reviewModal?.removeEventListener('show.bs.modal', this.loadReview)
  },
  watch: {
    async selectedImdbId(newImdbID) {
      this.text = ''
      if (!newImdbID || !this.userId) {
        return
      }
  ...
  methods: {
    async loadReview() {
      this.text = ''
      if (!this.userId || !this.currentImdbId) {
        return
      }

      try {
        const response = await axios.get(`/users/${this.userId}/watchedlist/${this.currentImdbId}/review`)
        this.text = response.data?.review || ''
      } catch (error) {
    ...
    async handleReview() {
      ...
      if (this.text.length < 5) {
        ...
        await axios.post(`/users/${this.userId}/watchedlist/${this.currentImdbId}/review`, {
          review: this.text
        })

...
export default {
  data() {
    return {
      ...
      savedMovieIds: [], // Tracks movies saved in the current result list
  ...
  methods: {
    async fetchMovies() {
      const searchQuery = this.search_s.trim()
      ...
      // A new search should always start with a clean list and Save button state.
      this.movies = []
      this.savedMovieIds = []
    ...
    async saveMovie(id, title, year, type, poster) {
      try {
        const response = await axios.post(`/users/${this.user.id}/watchlist`, {
          imdbID: id,
          Title: title,
          Year: year,
          Type: type,
        ...
        if (!this.savedMovieIds.includes(id)) {
          this.savedMovieIds.push(id)
        }
      } catch (error) {
    ...
    async handleSaveClick(event) {
      ...
      if (target.classList.contains('save')) {
        ...
        await this.saveMovie(id, title, year, type, poster)
              ...
              <tbody>
                <tr v-for="movie in movies" :key="movie.imdbID">
                  ...
                  <td class="td_save">
                    <button
                      class="btn-sm save"
                      :class="{ saved: savedMovieIds.includes(movie.imdbID) }"
                      type="button"
                      ...
                      :disabled="!user || savedMovieIds.includes(movie.imdbID)"
                    >
                      {{ !user ? 'Login to Save' : savedMovieIds.includes(movie.imdbID) ? 'Saved' : 'Save' }}
                    </button>
                  </td>
                </tr>
              </tbody>
